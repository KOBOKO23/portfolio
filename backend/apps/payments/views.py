...
class MpesaCallbackView(APIView):
    ...
    def post(self, request):
        try:
            ...
            order = PreOrder.objects.filter(mpesa_merchant_request_id=merchant_request_id).first()
            if not order:
...
class StripeWebhookView(APIView):
    ...
    def post(self, request):
        ...
        if event['type'] == 'payment_intent.succeeded':
            ...
            if order_id:
                PreOrder.objects.filter(id=order_id).update(status='paid')
...
class OrderStatusView(APIView):
    ...
    def get(self, request, order_id):
        try:
            ...
            return Response({'success': True, 'data': PreOrderSerializer(order).data})
        except PreOrder.DoesNotExist:
            return Response({'success': False, 'error': {'message': 'Order not found'}}, status=404)

    def patch(self, request, order_id):
        """Frontend confirms Stripe payment success."""
        try:
